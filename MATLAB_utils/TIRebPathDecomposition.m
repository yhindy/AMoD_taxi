function [allpaths,pathnodeindex] = TIRebPathDecomposition(MCFlowOutput,N,M,Sources,Sinks,FlowsIn,FlowsOut)
...
zerothreshold=1e-8; %The flows we get are kinda garbage. So we consider anything below this threshold to be zero.
...
for s=1:S %For each source
    ...
    while (FlowsIn(s)>zerothreshold & sum(sum( MCFlowOutput(Flowkij(k,1,1):Flowkij(k,N,N)) )) ) %While there is flow left
        ...
        while (myflow>0 & ~sum(Sinks==nextpos & FlowsOut>zerothreshold) ) %HACK
